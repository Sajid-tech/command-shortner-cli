...
export class CommandExecutor {
    private getShell(): ShellConfig {
        const settings = configManager.getSettings();

        if (settings.shellOverride) {
            return { shell: settings.shellOverride, shellArgs: ['-c'] }; // Default arg for custom shell?
        }
    ...
    private translateCommand(command: string): string {
        ...
        // Specific edge cases
        if (cmd === 'time' && parts.length === 1) return 'echo %TIME%';
        if (cmd === 'ping' && command.includes('-c')) return command.replace('-c', '-n');
        if (cmd === 'touch') return `echo. > ${parts.slice(1).join(' ')}`;
    ...
    public async execute(command: string, silent = false): Promise<void> {
